...
// namespace for simulation {
  ...
  class simulation {
    ...
    void updateStates(double delta, double v) { // created by cpp_define_function
      ...
      // restoring the signals s10, s11, psi_dot, x, y, psi from the states 
      ...
      // calculating the block outputs in the following order s4, s5, s6, s13, s7, s8, s9, s15, s17
    ...
    void calcResults_1(double &x, double &y, double &psi, double &psi_dot, double v, double wheelbase, double delta) { // created by cpp_define_function
      ...
      // calculating the block outputs in the following order s10, s11, psi_dot, x, y, psi
      // that depend on v, wheelbase, delta
      // dependencies that require a state update are s13, s15, s17 
      ...
      // saving the signals s10, s11, psi_dot, x, y, psi into the states 
      s10__block_7 = s10;
      s11__block_8 = s11;
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    // set datatypes 
    ssSetInputPortDataType(S, 0, SS_DOUBLE); // delta (double)
    ssSetInputPortDataType(S, 1, SS_DOUBLE); // v (double)
    ssSetInputPortDataType(S, 2, SS_DOUBLE); // wheelbase (double)
    ...
    ssSetOutputPortDataType(S, 0, SS_DOUBLE); // x (double)
    ssSetOutputPortDataType(S, 1, SS_DOUBLE); // y (double)
    ssSetOutputPortDataType(S, 2, SS_DOUBLE); // psi (double)
    ssSetOutputPortDataType(S, 3, SS_DOUBLE); // psi_dot (double)
