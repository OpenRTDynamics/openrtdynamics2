...
class UndeterminedSignal(Signal):
    ...
    def toStr(self):
        ...
        if self.inherit_datatype_of_signal is not None:
            string = string + 'type inherit from ' + self.inherit_datatype_of_signal.toStr()
...
class BlockOutputSignal(Signal):
    ...
    def __init__(self,  sim, datatype = None, sourceBlock = None, sourcePort = None):
        ...
        # used to describe signals that have their original source inside of a embedded subsystem
        # but for code-generation the that signal is connected to the block that embedds the subsystem
        self.sourcePort_inEmbeddedSystem = None
        self.sourceBlock_inEmbeddedSystem = None
